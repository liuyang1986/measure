...
public class HRecyclerView extends RelativeLayout {
    ...
    //触发拦截手势的最小值
    private int mTriggerMoveDis=40;
    ...
    private View createHeadLayout() {
        ...
        headLayout.addView(leftLayout, 0, new ViewGroup.LayoutParams(Util.dip2px(context, mLeftViewWidth), Util.dip2px(context, mLeftViewHeight)));
    ...
    private TextView addListHeaderTextView(String headerName, int width, LinearLayout leftLayout) {
        ...
        leftLayout.addView(textView, width, Util.dip2px(context, 50));
        return textView;
    }
    ...
    public void setHeaderListData(String[] headerListData) {
        ...
        for (int i = 0; i < headerListData.length; i++) {
            mRightTitleWidthList[i] = Util.dip2px(context, mRightItemWidth);
        }
        mLeftTextWidthList = new int[]{Util.dip2px(context, mLeftViewWidth)};
